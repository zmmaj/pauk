...
static int extracted_list_count = 0;
...
int get_list_count(void) {
    return extracted_list_count;
}

const char* get_list_filename(int index) {
    if (index < 0 || index >= extracted_list_count) return NULL;
...
void store_list_for_extraction(lxb_dom_element_t *list_elem, const char *filename, const char *list_type) {
    ...
    if (extracted_list_count >= list_capacity) {
    ...
    // Store list
    lists_to_extract[extracted_list_count].list_elem = list_elem;
    lists_to_extract[extracted_list_count].filename = strdup(filename);
    lists_to_extract[extracted_list_count].list_type = strdup(list_type);
    lists_to_extract[extracted_list_count].list_index = extracted_list_count + 1;
    extracted_list_count++;
}
...
void cleanup_lists_storage(void) {
    for (int i = 0; i < extracted_list_count; i++) {
    ...
    extracted_list_count = list_capacity = 0;
}
